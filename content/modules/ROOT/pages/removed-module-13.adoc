...
A sharp eyed junior noticed that one of the console arguments we passed is already applied by `bootc-image-builder`.
On the build host, let's update the config file and remove the duplicate `tty0` entry.
...
Red Hat base images use several tags, a `latest` tag that is attached to the most recent image of a major version, a RHEL version tag eg `10.1` that is attached to the latest base of a particular minor version, and a UNIX epoch timestamped version tag eg `10.1-1763458003` that stays with a specific image. 
...
These tools will allow you to track static tags like `10.1-1763458003` for full control over when updates are applied, but still make updates easier by eliminating the toil of tracking and updating the tag names.
...
=== Test our chained rebuild
==== _Switch to the "Sec SSH session" tab_

Let's apply our latest image update using our new soft reboot flag.
[source,bash,role="execute",subs=attributes+]
----
...
....
Our change was to the kernel command line options which can only be affected by a full reboot, so `soft-reboot` will error.
...
----
....
  Staged image: registry-m4fv6.apps.ocpvdev01.rhdp.net/app-test:v2
        Digest: sha256:ec45f16e4201bd0cd771ef2e7783c3995a3e839c6c98a926efe2b4096da9236f (amd64)
       Version: 10.1 (2025-12-17T05:44:27Z)
     StateRoot: default
 Deploy serial: 0
        Staged: yes
        Commit: 3583442881f2fe5a8fa942800812d2a4e1f4057f3f67cbad97c845b8cee0a12e
   Soft-reboot: no
....

Notice the `Digest` of the staged image matches the output of our update command, but the `Soft-reboot` is set to `no` unlike the other images.
...
The atomic nature of the updates lets us try to soft-reboot all the time and be notified when a hard reboot is required.

== Core principles
Tags and other metadata are useful for humans to keep track of what images are in use and how they should be used.
Derived builds make for smaller, more targeted Containerfile definitions but can introduce overhead best suited for automated builds.


You can reboot the host and check on the app if you like, but this brings us to the end of today's exploration of image mode.
Over the course of these exercises, you've:

* created an image that has needed customizations to integrate with the standard management tools 
* applied compliance policy and security tools
* considered transient versus permanent packages
* derived an application image from a baseline
* created a multi-stage build to keep build files from final images

all while exploring the new tools and design that make image mode hosts unique.
We hope what you've learned here today helps you adapt image mode for your own environment and begin deploying image mode hosts in your fleet.
