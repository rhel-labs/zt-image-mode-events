...
In addition to simplifying updates and providing native rollback, operating RHEL in image mode also makes it simple to adopt application container development practices for operating system definitions. 

In the last lab, we imposed some structure on application containers.
...
[NOTE]
====
If the Build host tab still has you looking at the vm console. press `Ctrl + ]` to exit the serial console.
====
...
The OCI specification allows for multiple image and associated actions to be defined in a single Containerfile.
This is referred to as a multi-stage build and can be used to separate portions of the build while making them still accessible later in the process. 
...
The `WORKDIR` directive is what's creating the intermediate build files inside the image.
...
This defines the first stage of our new build process.
...
RUN semodule_package -o nginx_connect_flask_sock.pp -m nginx_connect_flask_sock.mo
----
<.> Using `AS` lets us access the contents of the stage later via that name
<.> We copy from the host `app` directory instead of a directory in the image
...
Finally, replace the old block (The block mentioned above, that starts with "WORKDIR" and ends with semanage) with this one that copies from the `policy` stage
[source,dockerfile,role="execute",subs=attributes+]
----
...
We'll continue the exploration of derived builds and reboots in the next exercise.

== Core principles
We've addressed creating images and hosts that are based on other images, allowing standard operating environments to be created and maintained centrally and used simply by other teams.
We looked at how to separate build requirements from runtime requirements in our Containerfile.
We also looked at different ways reboots can affect hosts.
