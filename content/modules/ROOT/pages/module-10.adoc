...
If we were to try to use switch again, `bootc` would let us know that the image was already the one it use so there's no change to be made.
Instead we'd use `update` since we used the same image.

However, there's another way we can turn back time with image mode hosts: `bootc rollback`.
...
No changes would be made to `/var` in a rollback, but we will pick up the changes in our workaround to create the new directories. 
