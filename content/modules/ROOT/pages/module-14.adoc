= Building your own base images (Extra credit)
...
Instead, let's build a minimal base image to compare against the Red Hat base we've been using so far.
...
<.> Housekeeping to appropriately identify this as a `bootc` image to container tools
...
[source,bash,role="execute",subs=attributes+]
----
time podman build -t minimal -f Containerfile.scratch --cap-add=all --security-opt=label=type:container_runtime_t --device /dev/fuse
----

NOTE: Scratch builds are extensive and can take upwards of 6 min
...
....
For all that time spent, you can see there's a significant size difference as the _minimal_ manifest only installed about 200 packages.
...
Same contents just a new source image.
[source,dockerfile,role="execute",subs=attributes+]
----
# Change to the new minimal profile base
...
[source,bash,role="execute",subs=attributes+]
----
podman run --rm -it httpd:base rpm -q openssh-server
----
....
...
[source,bash,role="execute",subs=attributes+]
----
podman run --rm -it httpd:min rpm -q openssh-server
----
....
...
However we're not necessarily picking up dependencies for installed packages, and we need to learn a new syntax.
[source,bash,role="execute",subs=attributes+]
----
podman images httpd
----
...
All of these options can be useful in the right use case, but the tradeoffs between control and usability need to be considered.
Not only for the build itself, but for possible downstream images, running hosts, and other teams who need to understand how to correctly use these images.
