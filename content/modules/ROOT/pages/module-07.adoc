...
Remeber, labels are for humans, so adding something about the compliance configuration that can be read from the registry or locally without starting or examining a running host is helpful.
...
[source,bash,role="execute",subs=attributes+]
----
skopeo inspect --format '{{json .Labels }}' docker://registry-t8nvd.apps.ocpvdev01.rhdp.net/base | jq
----
