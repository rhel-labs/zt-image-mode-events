...
Then we'll modify our Containerfile, removing the line where we endable just FAPolicyd, and replacing it with an entire policy. Notice that the cis_server_l1_customized.xml file that we generated with autotailor is used here.  This file will contain your modified policy, and can be re-used on other build that you would like to apply the same custom policy to. 

Open up the Containerfile, and delete this line:
Delete this:
....
