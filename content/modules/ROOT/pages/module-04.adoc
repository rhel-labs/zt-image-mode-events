...
For image mode systems, `/boot` is read-only, so we have support for passing kernel paramaters via configuration files in `/usr/lib/bootc/kargs.d` as drop-ins.
