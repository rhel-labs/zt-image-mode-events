...
_Return to the tab labeled "Ops SSH session" to proceed with the update_ 
...
[#testing]
== Testing the changes
Log back into the VM by hitting the `reconnect` button or the reload icon next to the tab title.
...
We can also test the `sudoers` rules and `bootc status` at the same time.
[source,bash,role="execute",subs=attributes+]
----
sudo bootc status
----
