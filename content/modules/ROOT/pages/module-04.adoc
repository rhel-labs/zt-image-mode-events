...
COPY console_kargs.toml /usr/lib/bootc/kargs.d/05-cloud-kargs.toml
----

Use the same command to build this updated image as the initial build.
Since we added new steps to the end of the Containerfile, `podman` will reuse the previously built layers rather than rebuild the entire image.
...
To create system credentials, we'll reuse the user login, but typically some sort of service account would be used.
In any case, the files are identical, and can be published in the image. This will allow a newly created image mode host to update immediately after install.

=== Distributing pull secrets

The credentials are on disk, but in the user cache.
The `podman login` command can also generate a file for a single registry. 
...
Since we've made a change to our definition, we need to rebuild the image.
[source,dockerfile,role="execute",subs=attributes+]
----
...
TIP: If the terminal picks up underlining from the output, run `reset` in the terminal to clear it 
...
==== Immutable vs mutable directories
...
----
....
/etc/ostree/auth.json: regular file, no read permission
....
