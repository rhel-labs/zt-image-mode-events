...
Rechunking addresses those that come from the build process.
...
Return to the home directory to for this exercise
...
RUN dnf -y install @editors
RUN dnf -y install @development-tools --exclude=openh264
RUN dnf -y install @headless-management
...
Push the control image to the registry.
[source,bash,role="execute",subs=attributes+]
----
...
[source,bash,role="execute",subs=attributes+]
----
virsh undefine security-vm --remove-all-storage
----
...
Fetching layers ██████████████████░░ 60/65 
 └ Fetching ███████░░░░░░░░░░░░░ 13.69 MiB/35.57 MiB (12.83 MiB/s) ostree chunk 7c8a4f6a342beffcf4018              
...
Also notice that the `bootc` output on the Sec VM specifies ostree chunk IDs instead of layer ID.
...
Reboot both VMs to apply the change and switch back to the build host.
...
----
....
layers already present: 74; layers needed: 2 (5.8 kB)
Fetched layers: 5.63 KiB in 3 se⠁ Deploying
....
