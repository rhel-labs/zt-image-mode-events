...
Reproducing the issue was extremely simple with a known starting point.
In fact, instead of starting with the Containerfile, we could have used the exact image for our testing and ensured reproducibility.
...
Using the git repo and a 'GitOps' mindset, we can do our work on a new branch to more easily review with the developers.
...
First up, move this to the baseline image you've been building and updating during this lab. 
...
Since we're moving to a baseline that's had a compliance policy applied and other changes from the Red Hat supplied original, we should make sure that this doesn't break anything. 
Let's just update the working Containerfile to use our base and test that first.
...
Since this is a new version of the application, let's add the tag `v2` to the image name.
Tags are another way to convey information to admins and users about the contents of an image.
[source,bash,role="execute",subs=attributes+]
----
...
The services look fine so check if the app http://hostinfo-{guid}.{domain}/[up and running?,window=_blank]

NOTE: The URL will automatically redirect to the new host in the lab configuration. You will get a `502 Bad Gateway` error if the application is not started.

The app is not reachable, so something changed with the new baseline.

The CIS profile https://complianceascode.github.io/content-pages/guides/ssg-rhel10-guide-cis_server_l1.html#xccdf_org.ssgproject.content_rule_service_firewalld_enabled[turned on the firewall,window=_blank] which wasn't configured in the original developer image.
Check if `http` is in the allowed services list.
...
Not, it's not.
Let's add it here and re-test to see if that fixes the issue.
[source,bash,role="execute",subs=attributes+]
----
sudo firewall-cmd --add-service http
----
Is the app http://hostinfo-{guid}.{domain}/[up and running?,window=_blank]

OK, all good now but we'll need to handle that in the new derived build.

Let's move to the next item on the list: what files are getting copied to the host as part of the application repository.
...
Let's create some directories that match those types.
...
The `etc` directory will mimic the `/etc` on the host machine like in our baseline. 
...
Remember we also inherit all of the configs from the baseline as well.
[source,bash,role="execute",subs=attributes+]
----
...
The concept is that for any one "final artifact", be that standard baseline or production host, would be in a single location.
This may vary from how you manage configurations today with tools that might use a modular approach to create automation, like ansible playbooks.
Since Containerfiles aren't particularly modular, we're following a pattern from the developers of one app per repo.

=== Updating the Containerfile
...
The end of the Containerfile should look like this:
...
=== Soft versus hard reboots

Having a serial console makes the next test more interesting, and luckily we can get one to our virtual machine.

[source,bash,role="execute",subs=attributes+]
----
virsh console security-vm
----

You may need to hit `Enter` to see the login prompt.
Once you do you can proceed to the next step.
If you like you can execute the next `bootc` instruction in this serial session instead of the web terminal.

==== _Switch to the "Sec SSH session" tab to test the latest build of the image._
...
Keep in mind, all of the applications will be restarted, so for large in-memory databases you may not find soft reboots as unintrusive.

The `required` flag tells bootc to error if a soft reboot isn't possible. 
The `auto` flag tells bootc to execute whichever reboot is applicable. 
[source,bash,role="execute",subs=attributes+]
----
...
==== _Click on the tab labeled "Build host"_ 
Once you see "Deploying" in the output, switch to the build host and check the output of our serial console.
So far, the reboots of our VMs have been fairly quick, but you can scroll through the output to see that this reboot is different.
Not only was it quicker, but after `systemd` shut the system down, we never got passed to the BIOS.

To exit the `virsh console` hit `Ctrl+]`

==== _Switch to the "Sec SSH session" tab_
...
The output of `bootc status` will also show if a particular image is a soft reboot candidate with a new field in the verbose output.
[source,bash,role="execute",subs=attributes+]
----
...
[source,bash,role="execute",subs=attributes+]
----
systemctl status info-app.service nginx.service --no-pager
----
...
....
total 60
drwxr-xr-x.  5 root root  310 Jan  1  1970 .
drwxr-xr-x. 14 root root 4096 Jan  1  1970 ..
-rw-r--r--.  1 root root 7649 Jan  1  1970 README.md
-rw-r--r--.  1 root root 2482 Jan  1  1970 app.py
-rw-r--r--.  1 root root  273 Jan  1  1970 config.json
drwxr-xr-x.  5 root root   85 Jan  1  1970 etc
-rw-r--r--.  1 root root 6513 Jan  1  1970 helpers.py
-rw-r--r--.  1 root root  985 Jan  1  1970 nginx_connect_flask_sock.mo
-rw-r--r--.  1 root root 1001 Jan  1  1970 nginx_connect_flask_sock.pp
-rw-r--r--.  1 root root  234 Jan  1  1970 nginx_connect_flask_sock.te
...
The config files are moved, but the intermediate SELinux build files are still there.
Let's explore that in the next exercise.
