...
if [ ! -d ~/bootc-base ]; then
    ...
    echo "HINT: Create directory with: mkdir -p ~/bootc-base"
    exit 1
fi
...
if [ ! -d etc/sudoers.d ] || [ ! -d etc/ostree ] || [ ! -d usr/lib/bootc/kargs.d ]; then
    echo "FAIL: Required directory structure missing in bootc-base"
    echo "HINT: Create directories: etc/sudoers.d, etc/ostree, usr/lib/bootc/kargs.d"
    exit 1
fi

# Check that Containerfile exists
if [ ! -f Containerfile ]; then
    echo "FAIL: Containerfile should exist in ~/bootc-base"
    echo "HINT: Create Containerfile with bootc image definition"
    exit 1
fi

# Check that Containerfile has required content
if ! grep -q "FROM registry.redhat.io/rhel10/rhel-bootc" Containerfile; then
    echo "FAIL: Containerfile should use rhel-bootc base image"
    exit 1
fi

if ! grep -q "pcp-zeroconf" Containerfile; then
    echo "FAIL: Containerfile should install pcp-zeroconf and other admin tools"
    exit 1
fi

if ! grep -q "epel-release" Containerfile; then
    echo "FAIL: Containerfile should install EPEL repository"
    exit 1
fi
...
if ! grep -q "bootc-fetch-apply-updates.timer" Containerfile; then
    echo "FAIL: Containerfile should mask bootc-fetch-apply-updates.timer"
    exit 1
fi

# Check that auth.json exists in the right place
if [ ! -f etc/ostree/auth.json ]; then
    echo "FAIL: Registry auth file should exist at etc/ostree/auth.json"
    echo "HINT: Create with: podman login --authfile=auth.json then move to etc/ostree/"
    exit 1
fi

# Check that the base image exists locally
if ! podman image exists registry-${GUID}.${DOMAIN}/base; then
    echo "FAIL: Base image not found in local podman storage"
    echo "HINT: Build with: podman build --file Containerfile --tag registry-${GUID}.${DOMAIN}/base"
    exit 1
fi

# Check that the image was pushed to registry
if ! skopeo inspect docker://registry-${GUID}.${DOMAIN}/base > /dev/null 2>&1; then
    echo "FAIL: Base image not found in registry"
    echo "HINT: Push with: podman push registry-${GUID}.${DOMAIN}/base"
    exit 1
fi
