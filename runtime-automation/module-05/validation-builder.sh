...
if [ ! -f cis_server_l1_customized.xml ]; then
    echo "FAIL: Tailored SCAP policy should exist"
    echo "HINT: Run autotailor to create cis_server_l1_customized.xml"
    exit 1
fi

# Check that Containerfile has security packages
if ! grep -q "fapolicyd" Containerfile; then
    echo "FAIL: Containerfile should install security packages"
    echo "HINT: Add RUN dnf install for security tools"
    exit 1
fi

# Check that Containerfile has oscap-im
if ! grep -q "oscap-im" Containerfile; then
    echo "FAIL: Containerfile should use oscap-im for policy application"
    echo "HINT: Add oscap-im command with tailored policy"
    exit 1
fi

# Verify build completed (check for log or recent image)
if [ ! -f /tmp/module-05-build.log ]; then
    echo "FAIL: Image should be built with security policy"
    exit 1
fi
